...
try {
    ...
    Invoke-TestCase 'duplicate pack selection is rejected' {
        $packId = 'us.asme-b1.20.7.garden-hose'
        $result = Invoke-TestCli @('list', '-Pack', "$packId,$packId")
    ...
    Invoke-TestCase 'dry-run performs no writes' {
        $result = Invoke-TestCli @('install', '-ThreadDataPath', $threadData, '-StateRoot', $stateRoot, '-DryRun')
        Assert-Equal 0 $result.ExitCode $result.Output
    ...
    Invoke-TestCase 'fresh install copies exact bytes and writes a receipt' {
        $result = Invoke-TestCli @('install', '-ThreadDataPath', $threadData, '-StateRoot', $stateRoot)
        Assert-Equal 0 $result.ExitCode $result.Output
        ...
        Assert-Equal 'us.asme-b1.20.7.garden-hose' $receipt.packId 'Wrong receipt pack id.'
        Assert-Equal '1.0.0' $receipt.packVersion 'Wrong receipt pack version.'
    }
    ...
    Invoke-TestCase 'modified managed file is protected by default' {
        ...
        $result = Invoke-TestCli @('update', '-ThreadDataPath', $threadData, '-StateRoot', $stateRoot)
    ...
    Invoke-TestCase 'forced update backs up and replaces a modified managed file' {
        $result = Invoke-TestCli @('update', '-ThreadDataPath', $threadData, '-StateRoot', $stateRoot, '-Force')
        Assert-Equal 0 $result.ExitCode $result.Output
    ...
    Invoke-TestCase 'dry-run uninstall preserves files and state' {
        $result = Invoke-TestCli @('uninstall', '-ThreadDataPath', $threadData, '-StateRoot', $stateRoot, '-DryRun')
        Assert-Equal 0 $result.ExitCode $result.Output
    ...
    Invoke-TestCase 'normal uninstall removes a manager-created file' {
        $result = Invoke-TestCli @('uninstall', '-ThreadDataPath', $threadData, '-StateRoot', $stateRoot)
        Assert-Equal 0 $result.ExitCode $result.Output
    ...
    Invoke-TestCase 'identical unmanaged file requires explicit adoption and survives uninstall' {
        ...
        $result = Invoke-TestCli @('install', '-ThreadDataPath', $threadData, '-StateRoot', $stateRoot, '-AdoptExisting')
        Assert-Equal 0 $result.ExitCode $result.Output
        $result = Invoke-TestCli @('uninstall', '-ThreadDataPath', $threadData, '-StateRoot', $stateRoot)
        Assert-Equal 0 $result.ExitCode $result.Output
    ...
    Invoke-TestCase 'pack update removes an obsolete managed filename' {
        ...
        $result = Invoke-TestCli @('install', '-ThreadDataPath', $updateThreadData, '-StateRoot', $updateState)
        Assert-Equal 0 $result.ExitCode $result.Output
        ...
        $result = Invoke-TestCli -Arguments @('update', '-ThreadDataPath', $updateThreadData, '-StateRoot', $updateState) -CommandPath $copiedCli
        Assert-Equal 0 $result.ExitCode $result.Output
    ...
    Invoke-TestCase 'forced install plus uninstall restores unmanaged original' {
        $result = Invoke-TestCli @('install', '-ThreadDataPath', $threadData, '-StateRoot', $stateRoot, '-Force')
        Assert-Equal 0 $result.ExitCode $result.Output
